...
2. (CREATE) 사용자는 상품을 장바구니에 추가할 수 있다.
    ...
    -[ ] request: member의 인증 정보(회원일 경우)/session_id(비회원일 경우), product_item_id(상품명, 옵션), 가격, 수량, 품절여부
...
4. (DELETE) 사용자는 상품을 장바구니에 제거할 수 있다.
...
5. (UPDATE) 사용자는 각 상품의 구매 수량을 자유롭게 수정할 수 있다. ---> API가 필요없을지도?
...
6. 상품 품절 여부에 따라 구매 비활성화 처리를 할 수 있다.