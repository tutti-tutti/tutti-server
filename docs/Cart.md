...
1. (READ) 사용자는 장바구니에 담긴 모든 상품을 한 눈에 확인할 수 있다.
    -[X] 사용자가 장바구니 조회 api를 요청하면 delete_status=false 인 상품만 반환한다.
...
2. (CREATE) 사용자는 상품을 장바구니에 추가할 수 있다.
    -[X] 사용자가 장바구니 추가 api를 클릭하면 장바구니 리스트에 판매상품이 추가되어야 한다.
    -[X] request: member의 인증 정보(회원일 경우)~~/session_id(비회원일 경우)~~, product_item_id(상품명, 옵션, 가격, 품절여부),
     수량
    -[X] 그리고 전달 받은 DTO를 DB에 저장하려면 엔티티로 변환해야 한다.
    -[X] response: 없다.
    -[X] 사용자가 동일 상품을 추가할 경우, "장바구니에 동일한 상품이 있습니다. 장바구니에 추가하시겠습니까?" 메세지를 리턴하고 동의한다면 수량만 증가시킨다.
3. (DELETE) 사용자는 상품을 장바구니에 제거할 수 있다.
    -[X] 회원이 장바구니 상품의 삭제(버튼) api를 요청하면, delete_status=false인 단일 CartItem을 delete_status=true로 업데이트
     한다.
